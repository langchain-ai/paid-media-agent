...
await page.click("button:has-text('More providers')");
await page.waitForSelector(".options .option:has-text('Kimi')");
r.providers = await page.$$eval(".options .option .name", (els) => els.map((e) => e.textContent));
r.recommended = await page.$$eval(".options .option .flag .badge", (els) => els.map((e) => e.textContent));
...
r.pathRecommended = await page.$$eval(".options.two .pick .badge", (els) => els.map((e) => e.textContent));
await shot("06-path");
...
await page.click("button:has-text('Continue')");
await page.waitForSelector(".hero-title:has-text(\"You're set\")");
